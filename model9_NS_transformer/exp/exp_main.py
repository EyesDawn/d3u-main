...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            early_stopping( vali_loss, self.cond_pred_model, condition_path)
    ...
    def test(self, setting, test=0):
        ...
        sum_crps_sum =0.0 
        # QICE
        bin_count = np.zeros(self.model.diffusion_config.testing.n_bins)
        QICE_samples = 0
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                batch_bin_count, _, _ = compute_true_coverage_by_gen_QI(
                    config=self.model.diffusion_config, dataset_object=1,
                ...
                # if i % 10 ==0:
                #     y = batch_y[0,:,-1]
                #     res = y - y_T_mean.detach().cpu().numpy()[0,:,-1]
                #     distribution(y,res, name='./pic/res_distribution_f{i}')
        ...
        ## QICE
        n_bin = self.model.diffusion_config.testing.n_bins
