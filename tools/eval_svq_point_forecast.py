...
def evaluate_dataset(dataset_name: str, output_root: Path, gpu: int) -> dict:
    ...
    residual_all = y_true_all - y_hat_all

    mae, mse, rmse, mape, mspe = metric(y_hat_all, y_true_all)
    ...
    metrics = {
        ...
        "mape": float(mape),
        "mspe": float(mspe),
        "num_windows": int(y_true_all.shape[0]),
        "pred_len": int(y_true_all.shape[1]),
        "n_vars": int(y_true_all.shape[2]),
        "prediction_file": str(predictions_path.resolve()),
        "space": "scaled",
        "residual_definition": "y_true - y_hat",
        "feature_for_plot": "last variable",
    }
    save_metrics(dataset_dir / "metrics.json", metrics)
    print(
        f"[eval] {dataset_name}: mae={metrics['mae']:.6f}, mse={metrics['mse']:.6f}, "
        f"rmse={metrics['rmse']:.6f}, windows={metrics['num_windows']}"
    )
    return metrics


def plot_dataset(dataset_name: str, output_root: Path) -> Path:
    dataset_dir = output_root / dataset_name
    ...
    fig_path = dataset_dir / f"residual_{dataset_name}_hist_kde.png"
    ...
    if residual_last.size > 1 and residual_std > 1e-12:
        grid = np.linspace(residual_last.min(), residual_last.max(), 512)
        kde = gaussian_kde(residual_last)
        ax.plot(grid, kde(grid), color="#E45756", linewidth=2.0, label="KDE")
    else:
        kde_skipped = True

    ax.set_title(f"{dataset_name} Residual Distribution (Last Variable)")
...
def main() -> None:
    ...
    for dataset_name in cli_args.datasets:
        if not cli_args.plot_only:
            summary[dataset_name] = evaluate_dataset(dataset_name, output_root, cli_args.gpu)
        else:
            metrics_path = output_root / dataset_name / "metrics.json"
            if metrics_path.exists():
                summary[dataset_name] = json.loads(metrics_path.read_text(encoding="utf-8"))
        plot_dataset(dataset_name, output_root)

    if summary:
        save_metrics(output_root / "summary.json", summary)
        print(f"[done] summary saved to {output_root / 'summary.json'}")
